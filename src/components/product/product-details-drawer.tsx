    ...
    >
      <div className="">
        ...
        {/* {product.images.length === 1 && ( */}
          <Image src={product.images[0]} width={window.innerWidth} height={200} alt={product.name} />
        {/* )} */}
        {/* {product.images.length > 1 && (
      ...
      {/* <p className="text-sm font-semibold text-gray-600 dark:text-gray-300 mt-2">
        ...
        quidem sequi illum facere recusandae voluptatibus officia error
        provident voluptate.
      </p>
      <hr /> */}
      ...
      <div className="fixed bottom-0 left-0 right-0 flex w-full justify-between p-4 bg-white shadow-lg bg-gradient-to-r from-blue-100 to-blue-200 rounded-t-lg">
        <div className="flex items-center justify-center">
          <button onClick={() => setQuantity(quantity > 1 ? quantity - 1 : 1)} className="bg-gray-200 rounded-full h-8 w-8 flex items-center justify-center">
            <TbMinus className="h-6 w-6" />
          </button>
          <p className="text-lg font-semibold text-gray-700 dark:text-gray-200 mx-2">
            {quantity}
          </p>
          <button onClick={() => setQuantity(quantity + 1)} className="bg-gray-200 rounded-full h-8 w-8 flex items-center justify-center">
            <GoPlus className="h-6 w-6" />
          </button>
        </div>

        <Button onClick={handleAddProductToCart(product)} color="blue" className="flex gap-x-10">
          <p>Ajouter</p>
